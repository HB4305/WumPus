...
def write_output(file_path, agent, RES):
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        for step, (pos, action, point) in enumerate(RES, 1):
            f.write(
                f"Step {step:>2}: Pos {pos} - Action: {action:<15} | Point: {point}\n"
            )
...
def main():
    ...
    size, pit_prob, wumpus_count, game_mode, agent_mode = (
        config  # Updated to unpack 4 values
    )
    if game_mode == 0:
        # NORMAL mode
        if agent_mode == 0:
            ...
            # Show agent movement
            main_ui.showAgentMove(
                None, RESULT, MAPS, None, None, agent_mode=0, agent_index=0
            )
            ...
            env2.wumpus_positions = (
                original_wumpus_positions.copy()
            )  # Gán lại vị trí wumpus gốc
            ...
            main_ui.showAgentMove(
                None, RESULT1, MAPS1, None, None, agent_mode=1, agent_index=0
            )
            main_ui.showAgentMove(
                None, RESULT2, MAPS2, None, None, agent_mode=1, agent_index=1
            )

            write_output(
                file_path="output/result_agent1.txt", agent=agent1, RES=RESULT1
            )
            write_output(
                file_path="output/result_agent2.txt", agent=agent2, RES=RESULT2
            )
            return
    else:
        # ADVANCED mode
        if agent_mode == 0:
            ...
            print(len(list_env))
            main_ui.showAgentMove(
                None, RESULT, MAPS, None, list_env, agent_mode=0, agent_index=0
            )
            ...
            env2 = EnvironmentRandomAdvanced(
                size=size, k=wumpus_count, pit_prob=pit_prob
            )
            ...
            agent2 = AgentRandomAdvanced(env2, inference2)
            ...
            main_ui.showAgentMove(
                None, RESULT1, MAPS1, None, list_env1, agent_mode=1, agent_index=0
            )
            print("\n=== SHOWING AGENT 2 RESULTS ===")
            main_ui.showAgentMove(
                None, RESULT2, MAPS2, None, list_env2, agent_mode=1, agent_index=1
            )

            write_output(
                file_path="output/result_agent1_advanced.txt", agent=agent1, RES=RESULT1
            )
            write_output(
                file_path="output/result_agent2_advanced.txt", agent=agent2, RES=RESULT2
            )
            write_map_to_file(
                "output/wumpus_world_map_advanced_double.txt", original_grid
            )

            print(f"\n=== FINAL COMPARISON ===")
            print(
                f"Agent 1 (Advanced A*): Steps: {step_count1}, Score: {agent1.point}, Gold: {agent1.has_gold}, Escaped: {agent1.finished()}"
            )
            print(
                f"Agent 2 (Random): Steps: {step_count2}, Score: {agent2.point}, Gold: {agent2.has_gold}, Escaped: {agent2.finished()}"
            )
            return
