    ...
    def remove_wumpus_after_kill(self, agent_pos, agent_dir):
        ...
        while 0 <= current_x < self.size and 0 <= current_y < self.size:
            ...
            if pos in self.kb and self.kb[pos].get('possible_wumpus', False):
                ...
                # Cập nhật lại percepts các ô xung quanh từ environment
                for neighbor in get_neighbors(pos, self.size):
                    self._ensure_kb(neighbor)
                    percept = self.environment.get_percepts(neighbor[0], neighbor[1])
                    self.percepts[neighbor] = percept
                    # Nếu không còn stench thì xóa flag nguy hiểm
                    if not percept.get('stench', False):
                        self.kb[neighbor]['possible_wumpus'] = False
                        self.kb[neighbor]['safe'] = True
            break
        current_x += dx
        current_y += dy
        self._update_safety()
